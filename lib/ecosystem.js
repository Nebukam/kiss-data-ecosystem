...
const { POOL } = require(`@nkm/common`);
const { DATA_EVENT, DataBlock, Catalog } = require(`@nkm/data-core`);
...
class Ecosystem extends DataBlock {
    ...
    _Init() {
        ...
        fields.Subscribe(DATA_EVENT.ITEM_REGISTERED, this._OnFieldRegistered, this);
        fields.Subscribe(DATA_EVENT.ITEM_UNREGISTERED, this._OnFieldUnregistered, this);
        this._fields = fields;

        let models = POOL.Rent(ModelManager);
        models.ecosystem = this;
        models.Subscribe(DATA_EVENT.ITEM_REGISTERED, this._OnModelRegistered, this);
        models.Subscribe(DATA_EVENT.ITEM_UNREGISTERED, this._OnModelUnregistered, this);
        this._models = models;

        let entries = POOL.Rent(EntryManager);
        entries.ecosystem = this;
        entries.Subscribe(DATA_EVENT.ITEM_REGISTERED, this._OnEntryRegistered, this);
        entries.Subscribe(DATA_EVENT.ITEM_UNREGISTERED, this._OnEntryUnregistered, this);
    ...
    _OnEntityRegistered(p_entity) {
        p_entity.Subscribe(DATA_EVENT.DIRTY, this._OnEntityDirty, this);
        p_entity.Subscribe(DATA_EVENT.CLEANED, this._OnEntityCleaned, this);
        if (p_entity.dirty) {
    ...
    _OnEntityUnregistered(p_entity) {
        p_entity.Unsubscribe(DATA_EVENT.DIRTY, this._OnEntityDirty, this);
        p_entity.Unsubscribe(DATA_EVENT.CLEANED, this._OnEntityCleaned, this);
    }
