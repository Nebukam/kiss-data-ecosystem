...
const { COMMON_FLAGS } = require(`@nkm/common`);
const { STYLE } = require(`@nkm/style`);
const { DIALOG } = require(`@nkm/dialog`);
...
class ModelApply extends DataManipulationCommand {
    ...
    _InternalExecute() {
        ...
        var conf = { color: COMMON_FLAGS.WARNING, strong: true };
        var str0 = STYLE.TF(`No way back`, conf);
        var str1 = STYLE.TF(`${models.length} models`, conf);
        var str2 = STYLE.TF(`${eCount} entries`, conf);
        var str3 = STYLE.TF(`It cannot be undone.`, conf);
